...
import { RestaurantMenuSection } from "@/components/main/restaurant-menu-section"
import { StoreCard } from "@/components/main/stores-card"
import { Skeleton } from "@/components/ui/skeleton"
import { useFeaturedRestaurants, useMenuItems } from "@/hooks/queries/use-restaurants"
...
function RestaurantMenuRow({ restaurant }: { restaurant: RestaurantListItem }) {
	const { data: categories } = useMenuItems(restaurant.id)
	const items = (categories ?? []).flatMap((c) => c.items).slice(0, 8)
	...
	return (
		<>
			...
			{spotlightRestaurants.length > 0 && (
				<section className="vm-section">
					<SectionHeader
						title="Order by Restaurants"
						subtitle="Pick a vendor and order their campus favorites in a tap."
						href="/restaurants"
					/>
					<div className="space-y-8">
						{spotlightRestaurants.map((restaurant) => (
							<RestaurantMenuRow key={restaurant.id} restaurant={restaurant} />
						))}
					</div>
				</section>
			)}
