...
function ConversationRow({
  ...
  const lastText = conversation.lastMessage?.text || "Tap to start chatting"
  const hasUnread = conversation.unreadCount > 0
  const ts = conversation.lastMessage?.timestamp || conversation.createdAt
...
export function ConversationsSidebar() {
  ...
  const filtered = search
    ? conversations.filter((c) => {
        const name = [c.otherUser.firstName, c.otherUser.lastName, c.otherUser.name]
          .filter(Boolean)
          .join(" ")
          .toLowerCase()
