...
function getStoreProducts(store: Store, limit = 9): Product[] {
	...
	if (category.includes("electronics") || category.includes("laptop") || category.includes("phone")) {
		tokens.push("electronics", "laptop", "phone", "tech")
	}
	if (category.includes("fashion") || category.includes("sneaker") || category.includes("streetwear")) {
		tokens.push("fashion", "shirt", "sneaker", "streetwear", "jacket")
	}
	if (category.includes("book") || category.includes("stationery") || category.includes("print")) {
	...
	const matches = mockProducts.filter((product) => {
		const haystack = `${product.category.name} ${product.title} ${product.description}`.toLowerCase()
...
export default function Page() {
	...
	return (
		<div className="min-h-svh max-w-8xl mx-auto">
			<HeroSlider />
			...
			<section className="container mx-auto px-4 py-8">
				<SectionHeader
					...
					href="/restaurants"
				/>
				<div className="flex gap-4 overflow-x-auto pb-2 [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden md:grid md:grid-cols-2 md:overflow-visible xl:grid-cols-4">
			...
			<section className="container mx-auto px-4 py-8">
				...
				/>
				<div className="flex gap-3 overflow-x-auto pb-2 [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
					{(hostelEssentials.length ? hostelEssentials : mockProducts.slice(4, 12)).map((product) => (
						<ProductCard
							key={product.id}
							product={product}
							className="w-40 shrink-0 sm:w-45 lg:w-47.5"
