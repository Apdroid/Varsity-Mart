	...
	return (
		<div className="container mx-auto px-4 py-8">
			<div className="mx-auto max-w-2xl">
					...
					<>
						...
						<div>
							...
							<div className="space-y-3">
								...
								{recentFoodOrders.slice(0, 3).map((order) => (
									<Link
										key={order.orderId}
										href={`/orders/${order.orderId}`}
										className="flex items-center justify-between rounded-lg  bg-card p-4 transition-colors hover:bg-muted/50"
									>
										{/*<Image height={50} width={50} src={order.restaurantLogo || ""} alt={order.restaurantName || ""} />*/}
										<div className="leading-tight -gap-1.5">
											<p className="font-medium">{order.orderId} </p>
											<p className="text-sm font-bold">from {order.restaurantName}</p>
											<p className="text-sm text-muted-foreground">{formatDate(order.created_at || order.createdAt || "")}</p>
										</div>
										<span className={cn("rounded-full px-3 py-1 text-xs font-medium capitalize", getStatusColor(order.status))}>
											{order.status}
				...
				<Button
					variant="ghost"
					className="w-full text-destructive p-6 hover:bg-destructive bg-destructive/10 hover:text-destructive"
