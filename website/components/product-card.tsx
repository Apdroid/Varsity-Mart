  ...
  return (
    <div className={cn("group relative rounded-xl border border-border bg-card overflow-hidden", className)}>
      ...
      <Link href={`/products/${product.id}`} className="block aspect-square relative overflow-hidden">
        ...
        />
        {discount > 0 && <Badge className="absolute top-2 left-2 bg-red-500 text-white">{discount}% OFF</Badge>}
      ...
      <div className="absolute top-2 right-2 flex flex-col gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
        <Button size="icon" variant="secondary" className="h-8 w-8 rounded-full bg-background/80 backdrop-blur-sm">
          <Heart className={cn("h-4 w-4", product.isLiked && "fill-red-500 text-red-500")} />
        </Button>
      </div>
      ...
      <div className="p-3">
        <Link href={`/products/${product.id}`}>
          <h3 className="font-medium text-foreground line-clamp-2 text-sm leading-snug mb-1 hover:text-emerald-600 transition-colors">
            {product.title}
          </h3>
        </Link>

        <div className="flex items-center gap-2 mb-2">
          <span className="font-bold text-lg text-foreground">GH₵{product.price.toLocaleString()}</span>
